...
/* Four Spheres, a plane and a box */
#define BODYNUM 6
#define unifNUM 1
#define texNUM 6
bodyBody bodies[BODYNUM];
double unif[1] = {1};
double cAmbient[3] = {0.25, 0.25, 0.25};
texTexture texture1, texture2, texture3, texture4, texture5, texture6;
texTexture *textures[texNUM] = {&texture1, &texture2, &texture3, &texture4, &texture5, &texture6};
rayMaterial material;
lightLight lights[2];
meshMesh mesh;
/* Based on the uniforms, textures, rayIntersection, and texture coordinates, 
outputs a material with specular, diffuse and ambient light  */
...
/*Outputs a perfecr mirror material*/
void getMirrorMaterial (
        ...
        rayMaterial *material){
            ...
            material->hasTransmission = 0;
            double mirror[3] = {1.0, 1.0, 1.0};
            vecCopy(3, mirror, material->cMirror);
...
int initializeArtwork(void) {
    if ((texInitializeFile(&texture1, "imgimgimg.jpg") != 0) || (texInitializeFile(&texture2, "test2.jpg") != 0) ||
    (texInitializeFile(&texture3, "test3.png") != 0) || (texInitializeFile(&texture4, "orange.jpg") != 0) ||
    (texInitializeFile(&texture5, "tennis.jpg") != 0) || (texInitializeFile(&texture6, "img.jpg") != 0)) {
        return 1;
	}
    if(bodyInitialize(&bodies[0], sphUNIFDIM, 4, 1, sphGetIntersection, sphGetTexCoordsAndNormal, getPhongMaterial) != 0 ||
    bodyInitialize(&bodies[1], sphUNIFDIM, 4, 1, sphGetIntersection, sphGetTexCoordsAndNormal, getPhongMaterial) != 0 || 
    bodyInitialize(&bodies[2], sphUNIFDIM, 0, 1, sphGetIntersection, sphGetTexCoordsAndNormal, getMirrorMaterial) != 0 || 
    bodyInitialize(&bodies[3], sphUNIFDIM, 0, 1, sphGetIntersection, sphGetTexCoordsAndNormal, getMirrorMaterial) != 0 || 
    bodyInitialize(&bodies[4], plaUNIFDIM, 4, 1, plaGetIntersection, plaGetTexCoordsAndNormal, getPhongMaterial) != 0  || 
    bodyInitialize(&bodies[5], reshUNIFDIM, 4, 1, reshGetIntersection, reshGetTexCoordsAndNormal, getPhongMaterial) != 0){
        texFinalize(&texture1);
        texFinalize(&texture2);
        texFinalize(&texture3);
        texFinalize(&texture4);
        texFinalize(&texture5);
        texFinalize(&texture5);
        texFinalize(&texture6);
        return 2;
    }
    ...
    if(mesh3DInitializeBox(&mesh, 0, 0.5, 0, 0.5, 0, 1) != 0){
        texFinalize(&texture1);
        texFinalize(&texture2);
        texFinalize(&texture3);
        texFinalize(&texture4); 
    ...
    bodySetMaterialData(&bodies[5], &mesh);
    ...
    bodySetMaterialUniforms(&bodies[0], 0, materialUniforms, 4);
    bodySetMaterialUniforms(&bodies[1], 0, materialUniforms, 4);
    bodySetMaterialUniforms(&bodies[4], 0, materialUniforms, 4);
    bodySetMaterialUniforms(&bodies[5], 0, materialUniforms, 4);
    ...
    double translation[3] = {0.0, 0.0, 4};
...
void getSceneColor(
        ...
        const double d[3], double rgb[3], int recDepth) {
    double bound = rayINFINITY, texCoords[2], normal[3], sample[3];
    rayIntersection intersection;
    int winningI = -1;
    ...
    if(bestIntersection.t != rayNONE){
        ...
        double dNew[3], dNormal[3], dScaled, x[3], td[3];
        vecScale(3, bestIntersection.t, d, td);
        vecAdd(3, p, td, x);
        if(material.hasMirror == 1){
            double mirrorRGB[3], modulatedRGB[3];
            dScaled = 2.0 * vecDot(3, d, normal);
            vecScale(3, dScaled, normal, dNormal);
            vecSubtract(3, d, dNormal, dNew);
            if(recDepth > 0){
                getSceneColor(bodyNum, bodies, cAmbient, lightNum, lights, x, dNew, mirrorRGB, -1);
            }
            vecModulate(3, mirrorRGB, rgb, modulatedRGB);
            vecAdd(3, modulatedRGB, rgb, rgb);
        ...
        if((material.hasDiffuse == 1|| material.hasSpecular == 1)){
            ...
            for(int i = 0; i < lightNum; i++){
                ...
                if(getSceneShadow(bodyNum, bodies, x, lightEffects.uLight) == 1){
                    continue;
                }
                //Calculates iDiffuse
...
void render(void) {
    ...
    for (int i = 0; i < SCREENWIDTH; i += 1) {
        ...
        for (int j = 0; j < SCREENHEIGHT; j += 1) {
            ...
            double rgb[3];
            getSceneColor(BODYNUM, bodies, cAmbient, 2, lights,  p, d, rgb, 3);
